...
![](../images/碰撞触发器_mh8w69rzuc3i_1添加组件.png)
...
| --- | --- |
| 配置参数 | 说明 |
| <span style="color: blue; font-style: italic;">初始生效触发器</span> | 枚举实体运行时默认生效的碰撞触发器  支持通过下拉箭头修改触发器后面的枚举勾选方框来修改触发器的默认生效状态，修改后的生效状态会同步到对应触发器的详细编辑页内 |
| <span style="color: blue; font-style: italic;">\触发器列表</span> | 枚举实体配置的所有碰撞触发器 |


## **3.碰撞触发器的编**辑

![](../images/碰撞触发器_mh8w69rzuc3i_3碰撞触发器的编辑.png)

关卡编辑窗口中，实体会显示当前选中的碰撞触发器范围。

### **(1)碰撞触发器组**

![](../images/碰撞触发器_mh8w69rzuc3i_1碰撞触发器组.png)

* 枚举该实体预定义的所有碰撞触发器
* 通过点击![](../images/碰撞触发器_mh8w69rzuc3i_1碰撞触发器组_2.png)，添加碰撞触发器
* “序号:X”，X为“触发器序号”，作为节点输入项，可以调整碰撞触发器的参数

### (2)碰撞触发器基础信息

![](../images/碰撞触发器_mh8w69rzuc3i_2碰撞触发器基础信息.png)


|  |  |
| --- | --- |
| 配置参数 | 说明 |
| <span style="color: blue; font-style: italic;">初始生效</span> | 若开启，则该碰撞触发器随实体创建立即生效 |
| <span style="color: blue; font-style: italic;">生效目标</span> | 仅对配置的实体类型，进行碰撞触发源的检测  提供*角色、物件、造物*枚举 |


### (3)碰撞触发器范围

![](../images/碰撞触发器_mh8w69rzuc3i_3碰撞触发器范围.png)

通过“添加触发区”可新增触发区域，同一碰撞触发器下的范围会取并集范围生效

![](../images/碰撞触发器_mh8w69rzuc3i_3碰撞触发器范围_2.png)
...
![](../images/碰撞触发器_mh8w69rzuc3i_三通过节点图管理碰撞触发.png)
...
![](../images/碰撞触发器_mh8w69rzuc3i_三通过节点图管理碰撞触发_2.png)

* **激活/关闭碰撞触发器**

“碰撞触发器”组件功能支持通过节点图动态调整生效参数

![](../images/碰撞触发器_mh8w69rzuc3i_三通过节点图管理碰撞触发_3.png)
