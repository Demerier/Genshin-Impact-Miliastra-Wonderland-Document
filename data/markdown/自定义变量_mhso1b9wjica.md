...
自定义变量使用<span style="color: blue; font-style: italic;">自定义变量名</span>（字符串型）作为唯一索引，读取、修改、监听事件等操作均依赖自定义变量名称，因此在同一个实体上，自定义变量不能重名
...
仅有配置在组件中默认创建的自定义变量才会被同步至本地，被<span style="color: red; font-style: italic;">界面</span>、<span style="color: red; font-style: italic;">本地过滤器</span>等功能使用

自定义变量**不需要**定义后再使用。在服务端节点图中，可以使用<span style="color: blue; font-style: italic;">设置自定义变量节点</span>为实体动态创建新的自定义变量（但要确保新创建的自定义变量不与现存的重名）

定义过程中需要确定

* 自定义变量名：这是自定义变量的唯一标识。同一个组件内，自定义变量不允许重名
* *自定义变量<span style="color: blue; font-style: italic;">数据类型</span>：当前模式使用的是强类型，所有自定义变量定义时必须明确其类型。*
* 自定义变量的默认值：当实体创建时，该自定义变量会以这个默认值创建

![](../images/自定义变量_mhso1b9wjica_二自定义变量组件的功能.png)
...
![](../images/自定义变量_mhso1b9wjica_三自定义变量的编辑入口.png)

在组件页面可以看到当前已定义的自定义变量数

点击【详细编辑】进入自定义变量组件详细编辑页面

![](../images/自定义变量_mhso1b9wjica_三自定义变量的编辑入口_2.png)

**步骤2**：在自定义变量组件详细编辑页面新增或修改自定义变量

进入详细编辑后页面如下：

![](../images/自定义变量_mhso1b9wjica_三自定义变量的编辑入口_3.png)
...
![](../images/自定义变量_mhso1b9wjica_三自定义变量的编辑入口_4.png)

【新增自定义变量】：可以新增一个或若干个执行类型的自定义变量

![](../images/自定义变量_mhso1b9wjica_三自定义变量的编辑入口_5.png)

【其他功能】：关卡变量总览、筛选以及其他功能

![](../images/自定义变量_mhso1b9wjica_三自定义变量的编辑入口_6.png)

关卡自定义变量总览：可以查看当前关卡内所有实体上定义的自定义变量

![](../images/自定义变量_mhso1b9wjica_三自定义变量的编辑入口_7.png)

筛选：可以按照类型筛选已定义的自定义变量

![](../images/自定义变量_mhso1b9wjica_三自定义变量的编辑入口_8.png)

其他功能-批量删除：开启后可以批量删除自定义变量

注意，与修改相同的是，删除组件中定义的自定义变量也可能会导致节点图中自定义变量相关节点无法执行

![](../images/自定义变量_mhso1b9wjica_三自定义变量的编辑入口_9.png)
...
![](../images/自定义变量_mhso1b9wjica_1在服务端节点图中使用自定义变量.png)
...
![](../images/自定义变量_mhso1b9wjica_1在服务端节点图中使用自定义变量_2.png)
...
![](../images/自定义变量_mhso1b9wjica_1在服务端节点图中使用自定义变量_3.png)
...
![](../images/自定义变量_mhso1b9wjica_1在服务端节点图中使用自定义变量_4.png)

需要特别注意的是，所有<span style="color: red;">容器数据类型</span>的自定义变量变化事件中不会包含该变化的<span style="color: blue; font-style: italic;">变化前值</span>和<span style="color: blue; font-style: italic;">变化后值</span>，需要额外使用获取自定义变量节点来获取变化后的值

容器数据类型包括：所有<span style="color: red;">列表数据类型</span>、所有<span style="color: red;">字典数据类型</span>以及所有<span style="color: red;">结构体数据类型</span>

如下图所示

![](../images/自定义变量_mhso1b9wjica_1在服务端节点图中使用自定义变量_5.png)
...
![](../images/自定义变量_mhso1b9wjica_2在本地节点图中使用自定义变量.png)
...
节点图中的<span style="color: red; font-style: italic;">容器数据</span>类型的自定义变量有着较为特殊的引用传值的性质

容器数据类型包括：所有<span style="color: red; font-style: italic;">列表数据类型</span>、所有<span style="color: red; font-style: italic;">字典数据类型</span>以及所有<span style="color: red; font-style: italic;">结构体数据类型</span>

下文以列表数据类型为例

见下图：

![](../images/自定义变量_mhso1b9wjica_自定义变量与容器类型.png)
...
![](../images/自定义变量_mhso1b9wjica_自定义变量与容器类型_2.png)
...
![](../images/自定义变量_mhso1b9wjica_自定义变量与容器类型_3.png)

总而言之，节点图中**对容器类型自定义变量的操作，会导致自定义变量的值发生变化，而非容器类型的操作则不会**

此外，容器数据类型的<span style="color: blue; font-style: italic;">自定义变量变化时</span>事件中的<span style="color: blue; font-style: italic;">变化前值</span>和<span style="color: blue; font-style: italic;">变化后值</span>出参**是没有值的**，需要通过<span style="color: blue; font-style: italic;">获取自定义变量</span>节点来获取变化后的值

![](../images/自定义变量_mhso1b9wjica_自定义变量与容器类型_4.png)
