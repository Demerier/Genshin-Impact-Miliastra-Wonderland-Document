...
负责协调爬虫核心模块、解析器和下载器，完成原神千星奇域官方文档的爬取、解析和转换工作。
"""
...
def load_doc_id_map():
    ...
    try:
        ...
        logger.error("Failed to load doc_id_map: %s", e)
        return {}
...
def load_url_list():
    ...
    try:
        ...
        logger.error("Failed to load url_list: %s", e)
    return urls
...
def crawl_single_page(url, spider, parser, downloader, doc_id_map):
    ...
    try:
        ...
        # 保存Markdown文件
        filename = downloader.save_markdown(
            parsed_content['content'], title, MARKDOWN_DIR, doc_id)
        if not filename:
        ...
        for img_url in images:
            downloaded_filename = downloader.download_image(
                img_url, IMAGES_DIR)
        ...
        # 更新Markdown文件中的图片链接和本地跳转链接
        if img_map or len(links) > 0:
            # 读取保存的Markdown文件
            ...
            if img_map:
                ...
                # 1. 处理空图片链接 - 基于位置的匹配
                empty_img_pattern = r'!\[\]\(\)'
                empty_img_matches = list(re.finditer(
                    empty_img_pattern, markdown_content))

                if empty_img_matches:
                    # 将img_map转换为列表，保持顺序
                    img_list = list(img_map.items())

                    # 确保图片数量和空链接数量匹配
                    min_count = min(len(empty_img_matches), len(img_list))

                    # 基于位置替换空图片链接
                    for i in range(min_count):
                        match = empty_img_matches[i]
                        img_url, local_filename = img_list[i]
                        local_path = f"../images/{local_filename}"

                        # 替换当前位置的空图片链接
                        start_pos = match.start()
                        end_pos = match.end()
                        markdown_content = markdown_content[:start_pos] + \
                            f"![]({local_path})" + markdown_content[end_pos:]

                # 2. 替换所有完整的图片链接
                for img_url, local_filename in img_map.items():
                    # 创建本地相对路径
                    local_path = f"../images/{local_filename}"
                    # 替换CDN链接为本地相对路径（支持带标题的图片）
                    markdown_content = markdown_content.replace(
                        f"![]({img_url})", f"![]({local_path})")
                    markdown_content = markdown_content.replace(
                        f"![{img_url}]({img_url})", f"![]({local_path})")
            ...
            if links:
                ...
                for link in links:
                    ...
                    if doc_id in doc_id_map:
                        title = doc_id_map[doc_id]
                        local_filename = f"{title}_{doc_id[:8]}.md"
                        # 创建本地相对路径
                        local_path = f"./{local_filename}"

                        # 提取基本URL部分，用于匹配不同格式的链接
                        base_url = "/ys/ugc/tutorial/detail/"
                        relative_link1 = f"/ys/ugc/tutorial//detail/{doc_id}"
                        relative_link2 = f"{base_url}{doc_id}"

                        # 使用正则表达式替换所有包含该链接的Markdown链接，无论链接文本是什么
                        import re
                        # 匹配所有包含该URL的Markdown链接
                        for url_pattern in [link, relative_link1, relative_link2]:
                            ...
                            link_pattern = re.compile(
                                r'\[([^\]]+)\]\(\s*' + escaped_url + r'\s*\)')
                            # 替换为本地链接
                            markdown_content = link_pattern.sub(
                                rf'[\1]({local_path})', markdown_content)
...
def crawl_batch(url_list, spider, parser, downloader, doc_id_map, max_concurrent=5, retry_times=3):
    ...
    with ThreadPoolExecutor(max_workers=max_concurrent) as executor:
        ...
        future_to_url = {executor.submit(
            crawl_single_page, url, spider, parser, downloader, doc_id_map): url for url in url_list}
        ...
        for i, future in enumerate(as_completed(future_to_url), 1):
            ...
            try:
                ...
                if result["success"]:
                    ...
                    logger.info(
                        f"[{i}/{len(url_list)}] 爬取成功: {result['title']} ({result['content_length']} 字符)")
                else:
                    stats["failed"] += 1
                    ...
                    logger.error(
                        f"[{i}/{len(url_list)}] 爬取失败: {url} - {result['error']}")

                    # 重试逻辑
                    if retry_times > 0:
                        for retry in range(1, retry_times + 1):
                            logger.info(
                                f"[{i}/{len(url_list)}] 重试 {retry}/{retry_times}: {url}")
                            retry_result = crawl_single_page(
                                url, spider, parser, downloader, doc_id_map)
                            stats["retry_count"] += 1
                            ...
                            if retry_result["success"]:
                                ...
                                logger.info(
                                    f"[{i}/{len(url_list)}] 重试成功: {retry_result['title']}")
                                break
                            else:
                                logger.error(
                                    f"[{i}/{len(url_list)}] 重试 {retry}/{retry_times} 失败: {url} - {retry_result['error']}")
                ...
                logger.error(
                    f"[{i}/{len(url_list)}] 处理结果时出错: {url} - {e}", exc_info=True)
...
def main():
    ...
    try:
        ...
        if not url_list:
            logger.error(
                "No URLs to crawl. Please run initial_data_collection.py first.")
            return
        ...
        crawl_result = crawl_single_page(
            test_url, spider, parser, downloader, doc_id_map)

        if crawl_result["success"]:
            logger.info(
                f"Single page crawl test successful: {crawl_result['title']}")
            logger.info(
                f"  Content length: {crawl_result['content_length']} characters")
            logger.info(f"  Links found: {crawl_result['links_found']}")
            logger.info(f"  Images found: {crawl_result['images_found']}")
            logger.info(
                f"  Images downloaded: {crawl_result['images_downloaded']}")
        else:
            logger.error(
                f"Single page crawl test failed: {crawl_result['error']}")
            return
        ...
        batch_stats_1 = crawl_batch(
            url_list[:50], spider, parser, downloader, doc_id_map, max_concurrent=3)
        ...
        batch_stats_2 = crawl_batch(
            url_list[50:100], spider, parser, downloader, doc_id_map, max_concurrent=3)
        ...
        batch_stats_3 = crawl_batch(
            url_list[100:], spider, parser, downloader, doc_id_map, max_concurrent=3)
        ...
        logger.info(
            f"成功率: {total_stats['success'] / total_stats['total_urls'] * 100:.2f}%")
